            ...
            <table className="data-table">
                ...
                {filteredAndSorted.length <= VIRTUAL_THRESHOLD ? (
                    filteredAndSorted.map((r, idx) => {
                        ...
                        // ✅ [수정] 강조 표시(_highlight)와 검색(row-match) 클래스를 함께 적용
                        const classNames = [
                            isMatch ? "row-match" : "",
                            (r as any)._highlight ? "highlighted-row" : ""
                        ].join(" ").trim();

                        return (
                            <tr key={r.internalOrderId + "_" + idx} className={classNames || undefined}>
                                {headers.map((h) => (
                                    ...
                                    {({ index, style }) => {
                                        ...
                                        return (
                                            <div style={style}>
                                                <table className="data-table">
                                                    <tbody>
                                                    {/* ✅ [수정] key prop 추가 및 className 적용 */}
                                                    <tr key={r.internalOrderId + "_" + index} className={classNames || undefined}>
                                                        {headers.map((h) => (
                                                            <td key={h.key} className="data-td">
                                                                {String((r as any)[h.key] == null ? "" : (r as any)[h.key])}
                                                            </td>
